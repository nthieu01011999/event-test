...
public:	
	static std::vector<std::byte> ExtractSeqNALUS(uint8_t *buffers, uint32_t size);
	static std::vector<std::byte> ExtractSeqNALUS(const char *url, bool isCaptured = false);
};
