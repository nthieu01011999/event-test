...
std::vector<std::byte> H26XSource::ExtractSeqNALUS(const char *url, bool isCaptured) {
...
std::vector<std::byte> H26XSource::ExtractSeqNALUS(uint8_t *buffers, uint32_t size) {
	enum NALUType {
		Unspecified = 0,
		IDR = 5,
		SEI = 6,
	...
	std::vector<std::byte> idr, sps, pps;
	std::vector<std::byte> nalUnits;

    /* Search for IDR, SPS, and PPS NAL units */
	uint32_t ind = 0;
	while (ind < size) {
		if (buffers[ind] == 0x00 && buffers[ind + 1] == 0x00 && buffers[ind + 2] == 0x00 && buffers[ind + 3] == 0x01) {
            uint8_t nalType = buffers[ind + 4] & 0x1F;

			if (nalType != IDR && nalType != SPS && nalType != PPS) {
				++ind;
				continue;
			}

			auto &nalUnit = (nalType == IDR) ? idr : (nalType == SPS) ? sps : pps;
			nalUnit.push_back((std::byte)buffers[ind]);
			nalUnit.push_back((std::byte)buffers[ind + 1]);
			nalUnit.push_back((std::byte)buffers[ind + 2]);
			nalUnit.push_back((std::byte)buffers[ind + 3]);
			ind += 4;

			while (ind < size && !(buffers[ind] == 0x00 && buffers[ind + 1] == 0x00 && buffers[ind + 2] == 0x00 && buffers[ind + 3] == 0x01)) {
				nalUnit.push_back((std::byte)buffers[ind]);
				ind++;
			}
        }
	...
	if (isSeqFound) {
		...
		naluLen = htonl(static_cast<uint32_t>(sps.size() - 4));
		std::copy(reinterpret_cast<const std::byte *>(&naluLen), reinterpret_cast<const std::byte *>(&naluLen) + sizeof(naluLen), sps.begin());

		naluLen = htonl(static_cast<uint32_t>(pps.size() - 4));
		std::copy(reinterpret_cast<const std::byte *>(&naluLen), reinterpret_cast<const std::byte *>(&naluLen) + sizeof(naluLen), pps.begin());

		naluLen = htonl(static_cast<uint32_t>(idr.size() - 4));
		std::copy(reinterpret_cast<const std::byte *>(&naluLen), reinterpret_cast<const std::byte *>(&naluLen) + sizeof(naluLen), idr.begin());

		nalUnits.insert(nalUnits.end(), sps.begin(), sps.end());
		nalUnits.insert(nalUnits.end(), pps.begin(), pps.end());
		nalUnits.insert(nalUnits.end(), idr.begin(), idr.end());
	}
